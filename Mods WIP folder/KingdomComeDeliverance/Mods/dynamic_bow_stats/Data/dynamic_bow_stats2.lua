...
function DebugPlayerStructure()
    ...
    if System and System.Log then
        System.Log("=== Player Structure Debug ===")
        
        -- Verificar componente soul
        if player.soul then
            System.Log("Soul component: Found")
            System.Log("Soul properties:")
            
            -- Listar propriedades do soul
            for key, value in pairs(player.soul) do
                if type(value) == "number" then
                    System.Log("  " .. key .. " = " .. tostring(value))
                elseif type(value) == "function" then
                    System.Log("  " .. key .. " = [function]")
                else
                    System.Log("  " .. key .. " = [" .. type(value) .. "]")
                end
            end
        else
            System.Log("Soul component: NOT FOUND")
        end
        
        -- Verificar outros componentes relacionados a stats
        if player.rpgStats then
            System.Log("RPG Stats component: Found")
        else
            System.Log("RPG Stats component: NOT FOUND")
        end
        
        if player.playerStats then
            System.Log("Player Stats component: Found")
        else
            System.Log("Player Stats component: NOT FOUND")
        end
...
function DebugBowStats()
    ...
    if System and System.Log then
        System.Log("=== Debug Dynamic Bow Stats v1.9.6 ===")
        System.Log("Game State: " .. gameState)
        System.Log(playerStatus)
        System.Log(soulStatus)
